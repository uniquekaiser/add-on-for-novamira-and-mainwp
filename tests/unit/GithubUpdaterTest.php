...
final class GithubUpdaterTest extends TestCase {
	public function test_source_checkout_guard_and_fresh_update_hook_are_present(): void {
		$source = file_get_contents( dirname( __DIR__, 2 ) . '/includes/class-github-updater.php' );
		self::assertStringContainsString( "NOVAMIRA_MAINWP_DIR . '.git'", $source );
		self::assertStringContainsString( 'pre_set_site_transient_update_plugins', $source );
		self::assertStringContainsString( 'site_transient_update_plugins', $source );
	}
	...
	public function accepted_assets(): array {
		return array(
			'current'     => array( 'mainwp-novamira-addon-0.7.0.zip' ),
	...
	public function rejected_assets(): array {
		return array(
			...
			'source-new'  => array( 'add-on-for-novamira-and-mainwp-0.7.0.zip' ),
	...
	public function test_fresh_and_cached_update_rows_receive_complete_metadata(): void {
		...
		self::assertSame( '7.1', $result->response[ $key ]->tested );
		...
		self::assertSame( '7.1', $result->no_update[ $key ]['tested'] );
	}
}
